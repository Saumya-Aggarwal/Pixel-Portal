 ...
 *
 * It goes first, with no delay of its own. The lines are meant to be drawn
 * *out of* the diamond, and a mark that arrives after they have already
 * started reverses that reading: the rule appears to be assembling itself
 * around a point that is not there yet.
 */
const mark = {
 ...
 *
 * Held back just long enough for the mark to land, so the growth reads as
 * coming from it. The opacity is broken out and finishes early: a line that
 * fades over its whole travel is faintest at the outer end for the entire
 * animation, which fights the gradient already doing that job.
 */
const line = {
  ...
  visible: {
    ...
    transition: {
      ...
      ease: EASE.out,
      delay: 0.18,
      opacity: { duration: 0.3, delay: 0.18, ease: EASE.out },
    },
  },
} as const;
